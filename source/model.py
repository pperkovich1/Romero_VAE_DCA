...
class VAE(torch.nn.Module):
	def __init__(self, input_length, num_hidden, num_latent, activation_func):
		...
		self.z_mean = torch.nn.Linear(nums[-1], num_latent)
		self.z_log_var = torch.nn.Linear(nums[-1], num_latent)
		
		
		### DECODER
		self.hidden_out = torch.nn.ModuleList([nn.Linear(nums[i-1], nums[i-2])
